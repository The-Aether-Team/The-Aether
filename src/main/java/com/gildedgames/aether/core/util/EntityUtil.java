...
public class EntityUtil
{
    ...
    public static void spawnMovementExplosionParticles(Entity entity) {
        double d0 = entity.random.nextGaussian() * 0.02D;
        double d1 = entity.random.nextGaussian() * 0.02D;
        double d2 = entity.random.nextGaussian() * 0.02D;
        double d3 = 10.0D;
        double x = entity.getX() + ((double) entity.random.nextFloat() * entity.getBbWidth() * 2.0D) - entity.getBbWidth() - d0 * d3;
        double y = entity.getY() + ((double) entity.random.nextFloat() * entity.getBbHeight()) - d1 * d3;
        double z = entity.getZ() + ((double) entity.random.nextFloat() * entity.getBbWidth() * 2.0D) - entity.getBbWidth() - d2 * d3;
    ...
    public static void spawnSummoningExplosionParticles(Entity entity) {
        for (int i = 0; i < 20; ++i) {
            ...
            double d2 = entity.random.nextGaussian() * 0.02D;
            double d3 = 10.0D;
            double x = entity.getX(0.0D) - d0 * d3;
            double y = entity.getRandomY() - d1 * d3;
            double z = entity.getRandomZ(1.0D) - d2 * d3;
            entity.level.addParticle(ParticleTypes.POOF, x, y, z, d0, d1, d2);
        }
    }
}
