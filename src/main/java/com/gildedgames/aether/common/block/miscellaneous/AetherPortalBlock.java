...
public class AetherPortalBlock extends Block
{
	...
	public static void onBlockRightClicked(PlayerInteractEvent.RightClickBlock event) {
		...
		if (event.getItemStack().m_204117_(AetherTags.Items.AETHER_PORTAL_ACTIVATION_ITEMS)) {
		...
		if (fluidstate.getType() == Fluids.WATER && !blockstate.isAir()) {
			if (world.dimension() == Level.OVERWORLD || world.dimension() == AetherDimensions.AETHER_WORLD) {
				...
				for (Direction direction : Direction.values()) {
					if (world.getBlockState(pos.relative(direction)).m_204336_(AetherTags.Blocks.AETHER_PORTAL_BLOCKS)) {
		...
		protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.isEmptyBlock(this.world.getBlockState(blockpos))
					|| !(this.world.getBlockState(blockpos.below()).m_204336_(AetherTags.Blocks.AETHER_PORTAL_BLOCKS))) {
					break;
				}
			}

			BlockPos framePos = pos.relative(directionIn, i);
			return this.world.getBlockState(framePos).m_204336_(AetherTags.Blocks.AETHER_PORTAL_BLOCKS) ? i : 0;
		}
			...
			for (this.height = 0; this.height < 21; ++this.height) {
				for (int i = 0; i < this.width; ++i) {
					...
					if (i == 0) {
						...
						if (!(this.world.getBlockState(framePos).m_204336_(AetherTags.Blocks.AETHER_PORTAL_BLOCKS))) {
							break outerloop;
						}
					}
