        ...
        dispatcher.register(Commands.literal("aether")
                .then(Commands.literal("time").requires((commandSourceStack) -> commandSourceStack.hasPermission(2))
                        .then(Commands.literal("set")
                                .then(Commands.literal("day").executes((context) -> setTime(context.getSource(), 1000 * AetherTimeCapability.getTicksPerDayMultiplier())))
                                .then(Commands.literal("noon").executes((context) -> setTime(context.getSource(), 6000 * AetherTimeCapability.getTicksPerDayMultiplier())))
                                .then(Commands.literal("night").executes((context) -> setTime(context.getSource(), 13000 * AetherTimeCapability.getTicksPerDayMultiplier())))
                                .then(Commands.literal("midnight").executes((context) -> setTime(context.getSource(), 18000 * AetherTimeCapability.getTicksPerDayMultiplier())))
                                ...
                                .then(Commands.literal("day").executes((context) -> queryTime(context.getSource(), (int) (context.getSource().getLevel().getDayTime() / (AetherTimeCapability.getTicksPerDay()) % (long) Integer.MAX_VALUE))))
                        )
                )
        );
    }
