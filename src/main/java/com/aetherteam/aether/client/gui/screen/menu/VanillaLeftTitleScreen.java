...
public class VanillaLeftTitleScreen extends TitleScreen implements TitleScreenBehavior {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if ((roundedFadeAmount & -67108864) != 0) {
            ...
            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, fadeAmount);
            if (this.showMinceraftEasterEgg) {
                this.blitOutlineBlack(xOffset, 30, (x, y) -> {
                    this.blit(poseStack, x, y + yOffset, 0, 0, 99, 44);
                    this.blit(poseStack, x + 99, y + yOffset, 129, 0, 27, 44);
                    this.blit(poseStack, x + 99 + 26, y + yOffset, 126, 0, 3, 44);
                    this.blit(poseStack, x + 99 + 26 + 3, y + yOffset, 99, 0, 26, 44);
                    this.blit(poseStack, x + 155, y + yOffset, 0, 45, 155, 44);
                });
            } else {
                this.blitOutlineBlack(xOffset, 30, (x, y) -> {
                    this.blit(poseStack, x, y + yOffset, 0, 0, 155, 44);
                    this.blit(poseStack, x + 155, y + yOffset, 0, 45, 155, 44);
                });
            }
            RenderSystem.setShaderTexture(0, MINECRAFT_EDITION);
            blit(poseStack, xOffset + 88, 67 + yOffset, 0.0F, 0.0F, 98, 14, 128, 16);
        ...
        for (Widget renderable : this.renderables) { // Increases the x-offset to the left for image buttons if there are menu buttons on the screen.
