...
public class AetherMusicManager {
    ...
    public static void startPlaying(Music music) {
        ...
        if (isAetherBossMusic(music)) {
            currentMusic = MusicSoundInstance.forBossMusic(music.getEvent());
        } else {
            currentMusic = MusicSoundInstance.forMusic(music.getEvent());
        }
    ...
    public static boolean isAetherBossMusic(Music music) {
        Optional<ResourceKey<SoundEvent>> optionalResourceKey = Registry.SOUND_EVENT.getResourceKey(music.getEvent());
        return optionalResourceKey.filter(soundEventResourceKey -> Registry.SOUND_EVENT.getHolderOrThrow(soundEventResourceKey).is(AetherTags.SoundEvents.BOSS_MUSIC)).isPresent();
    }
    ...
    public static <T extends LivingEntity & AetherBossMob<?>> T getBossFromFight() {
        for (Map.Entry<UUID, LerpingBossEvent> event : getAetherBossFights().entrySet()) {
            ...
            Entity entity = minecraft.player.getLevel().getEntity(entityId);
