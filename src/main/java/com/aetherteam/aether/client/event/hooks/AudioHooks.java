...
public class AudioHooks {
    ...
    private static Holder<SoundEvent> getSoundEvent(SoundInstance sound) {
        SoundEvent soundEvent = Registry.SOUND_EVENT.get(sound.getLocation());
        if (soundEvent != null) {
            Optional<ResourceKey<SoundEvent>> optionalResourceKey = Registry.SOUND_EVENT.getResourceKey(soundEvent);
            if (optionalResourceKey.isPresent()) {
                return Registry.SOUND_EVENT.getHolderOrThrow(optionalResourceKey.get());
            }
        }
        return null;
    }
