...
public class BronzeDungeonBuilder {
    ...
    static {
        ROOM_OPTIONS_BUILDER.get("boss_room").add((manager, pos, rotation) -> new BronzeBossRoom(manager, "boss_room", pos, rotation), 1);
        ROOM_OPTIONS_BUILDER.get("chest_room").add((manager, pos, rotation) -> new BronzeDungeonRoom(manager, "chest_room", pos, rotation), 1);
        ROOM_OPTIONS_BUILDER.get("end_corridor").add((manager, pos, rotation) -> new BronzeTunnel(manager, "end_corridor", pos, rotation), 1);
        ROOM_OPTIONS_BUILDER.get("entrance").add((manager, pos, rotation) -> new BronzeDungeonRoom(manager, "entrance", pos, rotation), 1);
        ROOM_OPTIONS_BUILDER.get("lobby").add((manager, pos, rotation) -> new BronzeDungeonRoom(manager, "lobby", pos, rotation), 1);
        ROOM_OPTIONS_BUILDER.get("square_tunnel").add((manager, pos, rotation) -> new BronzeDungeonRoom(manager, "square_tunnel", pos, rotation), 1);
    }
    ...
    public void initializeDungeon(BlockPos startPos, Structure.GenerationContext genContext, StructurePiecesBuilder builder) {
        ...
        BronzeDungeonPiece bossRoom = this.chooseRoom("boss_room", startPos, rotation);
        ...
        if (direction != null) {
            ...
            BronzeDungeonPiece hallway = this.chooseRoom("square_tunnel", pos, bossRoom.getRotation());
            pos = BlockLogicUtil.tunnelFromEvenSquareRoom(hallway.getBoundingBox(), direction, this.nodeWidth);
            BronzeDungeonPiece defaultRoom = this.chooseRoom("chest_room", pos, hallway.getRotation());
    ...
    private boolean propagateRooms(StructurePiece currentNode, ChunkPos chunkPos, boolean placeLobby) {
        ...
        for (int i = 3; i > 0; i--) {
            ...
            if (this.hasConnection(currentNode, direction)) {
                ...
                BronzeDungeonPiece hallway = this.chooseRoom("square_tunnel", pos, rotation);
                pos = BlockLogicUtil.tunnelFromEvenSquareRoom(hallway.getBoundingBox(), direction, this.nodeWidth);
                BronzeDungeonPiece room = this.chooseRoom(roomName, pos, rotation);
    ...
    public boolean buildTunnelFromRoom(StructurePiece connectedRoom, List<StructurePiece> list, Rotation rotation, Direction direction, BlockPos origin) {
        ...
        BronzeDungeonPiece entrance = this.chooseRoom("entrance", startPos, rotation);
        list.add(entrance);
        ...
        do {
            ...
            BronzeDungeonPiece tunnel = this.chooseRoom("end_corridor", pos, rotation);
    ...
    public BronzeDungeonPiece chooseRoom(String name, BlockPos pos, Rotation rotation) {
        ...
        if (list != null) {
            ...
            if (option.isPresent()) {
                return option.get().provide(this.manager, pos, rotation);
            }
        }
        return new BronzeDungeonRoom(this.manager, name, pos, rotation);
    }
    ...
    public interface RoomProvider<T extends BronzeDungeonPiece> {
        T provide(StructureTemplateManager manager, BlockPos pos, Rotation rotation);
    }
