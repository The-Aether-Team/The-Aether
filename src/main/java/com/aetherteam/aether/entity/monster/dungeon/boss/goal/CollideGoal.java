...
public class CollideGoal extends Goal {
    ...
    public void tick() {
        ...
        for (Entity entity : this.slider.getLevel().getEntities(slider, collisionBounds)) {
            if (entity instanceof LivingEntity livingEntity && entity.hurt(AetherDamageTypes.entityDamageSource(this.slider.getLevel(), AetherDamageTypes.CRUSH, this.slider), 6)) {
                if (livingEntity instanceof Player player && player.getUseItem().is(Items.SHIELD) && player.isBlocking()) { // Disables the player's Shield if one is being used.
                    ...
                    this.slider.getLevel().broadcastEntityEvent(player, (byte) 30);
                }
