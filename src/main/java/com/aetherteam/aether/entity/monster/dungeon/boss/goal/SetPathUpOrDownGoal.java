...
public class SetPathUpOrDownGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (slider.getMoveDelay() != 1 || slider.getTargetPoint() != null || slider.getRandom().nextInt(3) != 0) {
            return false;
        }
    ...
    public void start() {
        ...
        AABB currentPath = calculatePathBox(slider.getBoundingBox(), targetPos.x() - currentPos.x(), 0, targetPos.z() - currentPos.z());
        ...
        for (int x = Mth.floor(currentPath.minX); x < currentPath.maxX; x++) {
            for (int z = Mth.floor(currentPath.minZ); z < currentPath.maxZ; z++) {
                BlockState state = slider.level().getBlockState(pos.set(x, targetPos.y(), z));
        ...
        slider.setMoveDirection(direction);
